...
デフォルトの検証対象OPは `samples/hono` です。OPはDocker内でサンプルアプリのビルド
...
指定できる値は `hono`, `express`, `fastify`, `nextjs` です。
...
- `CONFORMANCE_SAMPLE_APP`: `hono`, `express`, `fastify`, `nextjs` のいずれか。
...
- `sample_app`: `hono`, `express`, `fastify`, `nextjs` のいずれか。
...
Basic OP向けPKCE互換モード追加前の `samples/hono` に対するローカル実行では、以下が
エクスポートされました。
...
Basic OP向けPKCE互換モード追加後の `samples/hono` に対するローカル再実行では、以下が
エクスポートされました。
...
`samples/hono` に対するローカル再実行では、以下の結果になりました。
...
`samples/hono` に対するCI実行（GitHub Actions `OpenID Conformance` workflow）では、以下の
結果になりました。
...
hono と同じく screenshot 提出以外の未pass項目は0件であることを確認済み。
