...
## 直近の実行結果（2026-06-25, `samples/hono`, CI）

`oidcc-basic-certification-test-plan[server_metadata=discovery][client_registration=static_client]`
を `samples/hono`（`OIDC_ALLOW_NON_PKCE_AUTHORIZATION_CODE_FLOW=1` /
...
`samples/hono`（および他 sample）の OP に対する Basic OP static-client plan で、manual
